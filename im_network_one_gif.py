...
class BehaviorClone(object):
    ...
    def build_network(self, gif_pics):
        ...
        ext_pics = tf.reshape(ext_pic, [gif_len, cfg['extra_width'], cfg['extra_width'], self.img_d])
    ...
    def build_inputs_and_outputs(self, gif=None, ext=None, fdb=None, cmd=None): #, batch_gif_tensor):
        ...
        batch_ext_shape = [self.pic_num_each_gif, cfg['extra_width'], cfg['extra_width'], self.img_d]

        self.batch_gif = tf.placeholder(tf.float32, batch_gif_shape, name='batch_gif') if gif is None else gif
        self.batch_ext = tf.placeholder(tf.float32, batch_ext_shape, name='batch_ext') if ext is None else ext
