...
# print(load_poly_info.item().get('poly_type'))
    ...
    def reset_env(self):
        ...
        create_dir_not_exist('env_info')
        load_poly_info = self.get_train_color_dic()
        circle_color = load_poly_info.item().get('circle_color')
        ...
        self.env_dic = self.gen_random_poly_pic() if dic == None  else self.gen_random_poly_pic(dic)
        self.viewer.set_polys_property(self.env_dic)
        ...
        #----render----#
        self.viewer.render(True, [dic['choice'] ])
        img_np = self.viewer.ret_frame_np()

        self.step_img_list = []
        self.step_img_list.append(img_np)
        jpg_path = 'env_info/{:0>2d}.jpg'.format(self.env_step_id)  
        imageio.imsave(jpg_path, img_np)
        
        
    def step(self, v_x = None, v_y= None):
        self.env_step_id +=1
        choice = self.env_dic['choice']
        cho_poly = self.env_dic[choice]
        cho_poly['x']  = cho_poly['x'] + v_x
        cho_poly['y']  = cho_poly['y'] + v_y
            
        self.viewer.set_polys_property(self.env_dic)
        self.viewer.render(True,[choice] )
        img_np = self.viewer.ret_frame_np()
        ...
        if self.env_step_id==19:
            gif_path = 'env_info/all.gif'
            imageio.mimsave(gif_path, np.array(self.step_img_list))
